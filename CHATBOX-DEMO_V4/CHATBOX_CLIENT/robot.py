# robot.py — Robot client (sign-language-demo branch: pepeha on by default)
import argparse
import sys
import re
import subprocess
...
class SimpleConcurrentClient(BasicClient):
    ...
    • Intercepts intro requests and offers the Māori pepeha (on by default on this
      branch; --no-pepeha turns it off).
    """
    ...
    def __init__(self, config_file: str = "client_config.json",
        ...
        # On by default for the sign-language demo; --no-pepeha turns it off.
    ...
    def print_startup_info(self):
        ...
        print(f"  Pepeha   : {'on' if self.pepeha_enabled else 'off (--no-pepeha)'}")
        print()
        print("  Input modules :")
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--no-pepeha", dest="pepeha", action="store_false",
        help="turn off the te reo Māori pepeha intro (on by default on this branch)",
    )
    parser.add_argument("--pepeha", action="store_true", help=argparse.SUPPRESS)  # old flag, still accepted
    parser.add_argument(
